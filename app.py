...
if st.session_state.tab == "input":
    ...
    if st.session_state.step == 1:
        ...
        # ── Invisible real buttons (rendered first so they get stable keys) ──
        # Streamlit renders button key as part of element ID we can target via JS
        _hc1, _hc2, _hc3 = st.columns(3)
        with _hc1:
            if st.button("✏️ Teks Bebas", key="m_text", use_container_width=True):
        ...
        with _hc2:
            if st.button("📄 Dokumen", key="m_photo", use_container_width=True):
                ...
                st.rerun()
        with _hc3:
            if st.button("🗒️ Kombinasi", key="m_both", use_container_width=True):
                st.session_state.mode   = "both"
                st.session_state.imgs   = []
                st.session_state.pdfraw = b""
                st.rerun()

        # ── Beautiful clickable cards overlay (JS clicks the real buttons) ────
        st.markdown(f"""
        <style>
        /* Push the real button row up behind the cards */
        .mode-wrap {{ position:relative; margin-bottom:14px }}
        .mode-grid {{
        ...
        .mode-card {{
            ...
            padding:16px 8px 14px;text-align:center;background:#fff;
            cursor:pointer;transition:all .18s;user-select:none;position:relative;
        }}
        .mode-card:hover {{
            border-color:#555;background:#F9FAFB;
        ...
        <div class="mode-wrap">
          <div class="mode-grid">
            <div class="mode-card {'on' if m=='text'  else ''}" id="mc-text">
              <div class="mode-chk">✓</div>
              <span class="ic">✏️</span>
              <div class="lb">Teks Bebas</div>
              <div class="sb">Ketik langsung</div>
            </div>
            <div class="mode-card {'on' if m=='photo' else ''}" id="mc-photo">
              <div class="mode-chk">✓</div>
              <span class="ic">📄</span>
              <div class="lb">Dokumen</div>
              <div class="sb">PDF · Foto</div>
            </div>
            <div class="mode-card {'on' if m=='both'  else ''}" id="mc-both">
              <div class="mode-chk">✓</div>
              <span class="ic">🗒️</span>
        ...
        <script>
        (function() {{
          /* Map card id → substring of button label text */
          var labelMap = {{
            'mc-text':  '✏️ Teks Bebas',
            'mc-photo': '📄 Dokumen',
            'mc-both':  '🗒️ Kombinasi'
          }};

          function findAndClick(partialLabel) {{
            /* Search in parent window (Streamlit app frame) */
            var targets = [window, window.parent, window.top];
            for (var t of targets) {{
              try {{
                var btns = t.document.querySelectorAll('button');
                for (var b of btns) {{
                  var txt = b.innerText || b.textContent || '';
                  if (txt.trim().indexOf(partialLabel.trim()) !== -1) {{
                    b.click();
                    return true;
                  }}
                }}
          ...
          function attachHandlers() {{
            Object.keys(labelMap).forEach(function(id) {{
              ...
              el.onclick = function(e) {{
                e.stopPropagation();
                findAndClick(labelMap[id]);
              }};
            }});
          }}

          /* Attach now and retry after short delay (Streamlit may load async) */
          attachHandlers();
          setTimeout(attachHandlers, 800);
          setTimeout(attachHandlers, 1600);
        }})();
        </script>
        """, unsafe_allow_html=True)

        # Hide the real button row visually (still interactive via JS)
        st.markdown("""
        <style>
        /* Target the specific columns row containing mode buttons */
