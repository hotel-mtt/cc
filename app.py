...
def _app_login_wall():
    ...
    # ── Login page CSS ────────────────────────────────────────────────────────
    # Strategy: card top (logo+title) is HTML, then Streamlit widgets rendered
    # IMMEDIATELY below with negative margin-top to visually merge into the card.
    # No spacer, no outer wrapper with min-height that pushes things apart.
    st.markdown(f"""
<style>
...
<div class="lc-top">
  ...
  <div class="lc-sub">Mitra Tours &amp; Travel · Masukkan password</div>
  {f'<div class="lc-err">{_err}</div>' if _err else ''}
</div>
""", unsafe_allow_html=True)

    # Card bottom: Streamlit widgets rendered flush below the HTML card top
