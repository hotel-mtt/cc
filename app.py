...
_NAV_OPTIONS = ["Input", "Bulk Upload", "Dashboard", "Riwayat", "Pengaturan"]
_NAV_KEYS    = {"Input":"input","Bulk Upload":"bulk",
               "Dashboard":"dashboard","Riwayat":"log","Pengaturan":"settings"}
...
#  TAB — BULK UPLOAD
# =============================================================================
elif st.session_state.tab == "bulk":
 
    # ── Session state untuk bulk ──────────────────────────────────────────────
    ...
    st.markdown("""
    <style>
    .bulk-header{background:#111;border-radius:12px;padding:14px 16px;
        margin-bottom:14px;display:flex;align-items:center;gap:12px}
    .bulk-header-icon{font-size:22px;line-height:1}
    .bulk-header-text h3{color:#fff;font-size:15px;font-weight:700;margin:0;line-height:1.2}
    .bulk-header-text p{color:#888;font-size:11px;margin:4px 0 0}
    ...
    .fi-icon{width:34px;height:34px;border-radius:8px;display:flex;
        align-items:center;justify-content:center;font-size:16px;flex-shrink:0}
    .fi-icon-ok{background:#DCFCE7}
    .fi-icon-err{background:#FFE4E6}
    .fi-icon-skip{background:#FEF9C3}
    .fi-icon-neutral{background:#F3F4F6}
 
    .fi-name{font-size:12px;font-weight:600;color:#111;flex:1;
        overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
 
    .fi-badge{font-size:10px;font-weight:700;padding:3px 9px;border-radius:20px;white-space:nowrap}
    .fi-badge-ok{background:#DCFCE7;color:#166534}
    .fi-badge-err{background:#FFE4E6;color:#9F1239}
    .fi-badge-skip{background:#FEF9C3;color:#713F12}
 
    .fi-grid{margin-top:9px;padding-top:8px;border-top:1px solid #F3F4F6;
        display:grid;grid-template-columns:1fr 1fr;gap:4px 14px}
    .fi-kv{display:flex;gap:5px;align-items:baseline}
    .fi-k{font-size:10px;font-weight:700;color:#9CA3AF;min-width:55px;flex-shrink:0}
    .fi-v{font-size:11px;font-weight:500;color:#374151;
        overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
 
    .bulk-summary{background:#fff;border:1px solid #E5E7EB;border-radius:12px;
        padding:14px 16px;margin-bottom:14px}
    .bulk-sum-title{font-size:10px;font-weight:700;text-transform:uppercase;
        letter-spacing:.8px;color:#9CA3AF;margin-bottom:10px}
    .bulk-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;
        text-align:center;margin-bottom:10px}
    .bs-val{font-size:22px;font-weight:700;color:#111;line-height:1}
    .bs-lbl{font-size:10px;color:#9CA3AF;margin-top:3px}
    .bs-green{color:#22C55E} .bs-red{color:#EF4444} .bs-yellow{color:#EAB308}
 
    .bulk-bar{background:#F3F4F6;border-radius:99px;height:6px;overflow:hidden}
    .bulk-bar-fill{height:100%;background:#22C55E;border-radius:99px}
    .bulk-prog-wrap{margin:4px 0 10px}
    .bulk-prog-bar{background:#F3F4F6;border-radius:99px;height:5px;overflow:hidden;margin-bottom:5px}
    .bulk-prog-fill{height:100%;background:#111;border-radius:99px}
    .bulk-prog-lbl{font-size:10px;color:#9CA3AF;text-align:center}
    </style>
    """, unsafe_allow_html=True)
    ...
    <div class="bulk-header">
      <div class="bulk-header-icon">⚡</div>
      <div class="bulk-header-text">
        <h3>Bulk Upload</h3>
        <p>Upload banyak file sekaligus — AI membaca &amp; menyimpan otomatis ke Sheets</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ── Pengaturan Batch ──────────────────────────────────────────────────────
    st.markdown('<div class="sec-lbl">Pengaturan Batch</div>', unsafe_allow_html=True)
    ...
    bulk_issuer = bc1.selectbox(
        "Issuer *", options=_ISSUERS_B, index=_bi,
        format_func=lambda x: "— Pilih Issuer —" if x == "" else x,
        key="bulk_issuer",
    )
    bulk_pic = bc2.text_input(
    ...
    # ── File uploader ─────────────────────────────────────────────────────────
    st.markdown(
        '<div class="sec-lbl" style="margin-top:12px">Upload File (pilih banyak sekaligus)</div>',
        unsafe_allow_html=True,
    )
    ...
    ftypes_b = ["jpg", "jpeg", "png", "webp"] + (["pdf"] if _PDF_OK else [])
    ...
    n_files = len(bulk_files) if bulk_files else 0
    if n_files:
        notice("info", f"<b>{n_files} file</b> dipilih dan siap diproses.")
 
    skip_dup = st.checkbox(
        "Lewati duplikat — jangan simpan jika booking sudah ada di Sheets",
        value=True, key="skip_dup",
    )
 
    st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
 
    # ── Tombol ────────────────────────────────────────────────────────────────
    bta, btb = st.columns(2)
    run_bulk = bta.button(
        "⚡  Proses & Simpan Semua", type="primary",
        use_container_width=True,
        disabled=(not n_files or not bulk_issuer or not bulk_pic.strip()),
    )
    clear_bulk = btb.button(
    ...
    if run_bulk:
        if not bulk_issuer:
            ...
            for idx, uf in enumerate(bulk_files):
                pct = int(idx / n_files * 100)
                prog_slot.markdown(
                    '<div class="bulk-prog-wrap"><div class="bulk-prog-bar">'
                    + f'<div class="bulk-prog-fill" style="width:{pct}%"></div>'
                    + '</div>'
                    + f'<div class="bulk-prog-lbl">Memproses {idx + 1} / {n_files} &middot; {uf.name}</div>'
                    + '</div>',
                    unsafe_allow_html=True,
                )
 
                res = {"file": uf.name, "status": "error", "parsed": {}, "err": ""}
                ...
                try:
                    ...
                    if uf.name.lower().endswith(".pdf"):
                        ...
                        pages  = pdf_images(raw_bytes)
                        imgs_b = [to_b64(p) for p in pages]
                        ptxt_b = pdf_text(raw_bytes)
                    else:
                        img_obj = Image.open(io.BytesIO(raw_bytes)).convert("RGB")
                        b64, mime_ = to_b64(img_obj)
                        imgs_b = [(b64, mime_)]
                    ...
                    parsed, _ = ai_parse(combined_b, imgs_b or None)
                    parsed["timestamp_input"] = now_ts()
 
                    # Duplicate check
                    is_dup, dup_reason, _ = check_duplicate(
                        {
                    ...
                    if is_dup and skip_dup:
                        ...
                        row = {
                            "timestamp_input": parsed.get("timestamp_input", ""),
                            "supplier":        parsed.get("supplier",      ""),
                            "booking_id":      parsed.get("booking_id",    ""),
                            "booked_on":       parsed.get("booked_on",     ""),
                            "issued_on":       parsed.get("issued_on",     ""),
                            "hotel":           parsed.get("hotel",         ""),
                            "checkin":         parsed.get("checkin",       ""),
                            "qty":             parsed.get("qty",           ""),
                            "room":            parsed.get("room",          0),
                            "checkout":        parsed.get("checkout",      ""),
                            "name":            parsed.get("name",          ""),
                            "card":            parsed.get("card",          ""),
                            "issuer":          bulk_issuer,
                            "pic":             bulk_pic,
                            "no_bc":           parsed.get("no_bc",        ""),
                            "nama_kegiatan":   parsed.get("nama_kegiatan",""),
                        ...
                        # Tambah ke cache lokal agar dup check berikutnya akurat
                        existing_rows.append({
                            "Booking ID": parsed.get("booking_id", ""),
                            "Hotel":      parsed.get("hotel",      ""),
                            "Check-in":   parsed.get("checkin",    ""),
                            "Guest Name": parsed.get("name",       ""),
                            "Total (Rp)": parsed.get("room",       0),
                        })
            ...
            st.session_state.bulk_saved_count = saved_this_run
            st.rerun()
    ...
    if results:
        ...
        n_tot  = len(results)
        pct_ok = int(n_ok / n_tot * 100) if n_tot else 0
 
        # Summary card
        st.markdown(
            '<div class="bulk-summary">'
            '<div class="bulk-sum-title">Hasil Proses Batch</div>'
            '<div class="bulk-stats">' +
            f'<div><div class="bs-val">{n_tot}</div><div class="bs-lbl">Total File</div></div>' +
            f'<div><div class="bs-val bs-green">{n_ok}</div><div class="bs-lbl">Tersimpan</div></div>' +
            f'<div><div class="bs-val bs-red">{n_err}</div><div class="bs-lbl">Gagal</div></div>' +
            f'<div><div class="bs-val bs-yellow">{n_skip}</div><div class="bs-lbl">Duplikat</div></div>' +
            '</div>' +
            '<div class="bulk-bar">' +
            f'<div class="bulk-bar-fill" style="width:{pct_ok}%"></div>' +
            '</div>' +
            f'<div style="font-size:10px;color:#9CA3AF;margin-top:5px;text-align:right">{pct_ok}% berhasil tersimpan</div>' +
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        for r in results:
            ...
            _item_cls  = {"success": "fi-success",  "error": "fi-error",     "skipped": "fi-skipped"}.get(s, "")
            _icon_cls  = {"success": "fi-icon-ok",  "error": "fi-icon-err",  "skipped": "fi-icon-skip"}.get(s, "fi-icon-neutral")
            _badge_cls = {"success": "fi-badge-ok", "error": "fi-badge-err", "skipped": "fi-badge-skip"}.get(s, "fi-badge-ok")
            _icon_sym  = {"success": "✓", "error": "✕", "skipped": "⚠"}.get(s, "○")
            _label     = {"success": "Tersimpan", "error": "Gagal", "skipped": "Duplikat"}.get(s, s)
 
            fname = r["file"]
            ficon = "📄" if fname.lower().endswith(".pdf") else "🖼️"
 
            # PENTING: bangun detail_html tanpa leading whitespace/indentasi
            # agar Streamlit tidak salah parse sebagai code block markdown
            if p and s in ("success", "skipped"):
                _dup_warn = (
                    '<div style="margin-top:6px;font-size:11px;color:#92400E;'
                    'background:#FEF9C3;padding:5px 8px;border-radius:6px">'
                    f'&#9888; {r.get("err", "Duplikat")}</div>'
                ) if s == "skipped" else ""
 
                detail_html = (
                    '<div class="fi-grid">'
                    '<div class="fi-kv">'
                    '<span class="fi-k">Hotel</span>'
                    f'<span class="fi-v">{p.get("hotel") or "—"}</span>'
                    '</div>'
                    '<div class="fi-kv">'
                    '<span class="fi-k">Total</span>'
                    f'<span class="fi-v">{fmt(p.get("room", 0))}</span>'
                    '</div>'
                    '<div class="fi-kv">'
                    '<span class="fi-k">Tamu</span>'
                    f'<span class="fi-v">{p.get("name") or "—"}</span>'
                    '</div>'
                    '<div class="fi-kv">'
                    '<span class="fi-k">Booking ID</span>'
                    f'<span class="fi-v">{p.get("booking_id") or "—"}</span>'
                    '</div>'
                    '<div class="fi-kv">'
                    '<span class="fi-k">Check-in</span>'
                    f'<span class="fi-v">{p.get("checkin") or "—"}</span>'
                    '</div>'
                    '<div class="fi-kv">'
                    '<span class="fi-k">Supplier</span>'
                    f'<span class="fi-v">{p.get("supplier") or "—"}</span>'
                    '</div>'
                    '</div>'
                    + _dup_warn
                )
                ...
                detail_html = (
                    ...
                    '<div class="fi-kv">'
                    '<span class="fi-k">Error</span>'
                    f'<span class="fi-v" style="color:#EF4444;white-space:normal">{r["err"]}</span>'
                    '</div>'
                    '</div>'
                )
            else:
            ...
            # Render satu blok HTML tanpa indentasi di awal
            st.markdown(
                '<div class="file-item ' + _item_cls + '">'
                '<div class="fi-top">'
                '<div class="fi-icon ' + _icon_cls + '">' + ficon + '</div>'
                '<div class="fi-name">' + fname + '</div>'
                '<span class="fi-badge ' + _badge_cls + '">' + _icon_sym + ' ' + _label + '</span>'
                '</div>'
                + detail_html +
                '</div>',
                unsafe_allow_html=True,
        ...
        if sid and n_ok:
            st.link_button(
                f"📊  Buka Google Sheets → ({n_ok} baris baru tersimpan)",
                f"https://docs.google.com/spreadsheets/d/{sid}",
                use_container_width=True,
            )
        ...
        if n_err:
            notice("warn",
                f"{n_err} file gagal diproses. "
                "Periksa kualitas gambar atau coba upload ulang manual via tab Input.")
...
# =============================================================================
    ...
    try:
        ...
        if not rows:
            ...
            st.markdown('<div class="sec-lbl" style="margin-top:14px"></div>',
                    unsafe_allow_html=True)
